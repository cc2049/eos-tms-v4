  ...
  <el-drawer :append-to-body="true" title="条件设置" v-model="visible" direction="rtl" custom-class="condition_copyer set_promoter" size="550px" :before-close="saveCondition">
    ...
    <div class="demo-drawer__content">
      <div class="condition_content drawer_content">
        ...
        <ul>
          <li v-for="(item,index) in conditionConfig.conditionList" :key="index" class="condition-item">
            <el-input type=textarea v-model="item.expression" :rows="3" placeholder="请输入条件" style="width:100%;margin-bottom:20px"></el-input>
            <el-input type=textarea v-model="item.showName" :rows="3" placeholder="请输入条件描述" style="width:100%"></el-input>
            <template v-if="false">
              <span class="ellipsis">{{item.type==1 ? '发起人':item.showName}}：</span>
              <el-input v-model="item.expression" :placeholder="'请输入'+item.showName"></el-input>
              <div>
                <div v-if="item.type==1">
                  <p :class="conditionConfig.nodeUserList.length > 0?'selected_list':''" @click.self="addConditionRole" style="cursor:text">
                    <span v-for="(item1,index1) in conditionConfig.nodeUserList" :key="index1">
                      {{item1.name}}<img src="@/assets/images/workflow/add-close1.png" @click="$func.removeEle(conditionConfig.nodeUserList,item1,'targetId')">
                    </span>
                ...
                <div v-else-if="item.columnType == 'String' && item.showType == 3">
                  <p class="check_box">
                    <a :class="$func.toggleStrClass(item,item1.key)&&'active'" @click="toStrChecked(item,item1.key)" v-for="(item1,index1) in JSON.parse(item.fixedDownBoxValue)" :key="index1">{{item1.value}}</a>
                  </p>
                </div>
                <div v-else>
                  <p>
        ...
        <el-dialog v-if="false" title="选择条件" v-model="conditionVisible" width="480px" append-to-body custom-class="condition_list">
          ...
          <p class="check_box">
            ...
            <a v-for="(item,index) in conditions" :key="index" :class="$func.toggleClass(conditionList,item,'columnId')&&'active'" @click="$func.toChecked(conditionList,item,'columnId')">{{item.showName}}</a>
          </p>
          <template #footer>
      ...
      <employees-role-dialog v-if="false" v-model:visible="conditionRoleVisible" v-model:data="checkedList" @change="sureConditionRole" :isDepartment="true" />
...
const addCondition = async () => {
  conditionConfig.value.conditionList.push({
    "showType": "1",
    "columnId": conditionConfig.value.conditionList.length * 1 + 1,
    "type": 2,
    "showName": "自定义" + (conditionConfig.value.conditionList.length * 1 + 1),
    "zdy1": "",
    "columnDbname": "",
    "columnType": "",
    "fixedDownBoxValue": "",
