  ...
  <el-drawer :append-to-body="true" title="审批人设置" v-model="visible" direction="rtl" class="set_promoter" size="550px" :before-close="saveApprover">
    <div class="demo-drawer__content">
      <div class="drawer_content">
        <div class="approver_content">
          ...
          <p class="selected_list" v-if="approverConfig.settype==1">
            <span v-for="(item,index) in approverConfig.nodeUserList" :key="index">{{item.name}}
              <img src="@/assets/images/workflow/add-close1.png" @click="$func.removeEle(approverConfig.nodeUserList,item,'targetId')">
            </span>
        ...
        <div class="approver_manager" v-if="approverConfig.settype==8">
          <p style="padding-bottom:20px">
            <span>相关业务的：</span>
            <select v-model="approverConfig.directorLevel">
              <option v-for="item in 2" :value="item" :key="item">{{item==1?'销售':'其他'}}</option>
            </select>
          </p>
        </div>
        <div class="approver_some" v-if="(approverConfig.settype==1&&approverConfig.nodeUserList.length>1)||approverConfig.settype==2||(approverConfig.settype==4&&approverConfig.selectMode==2)">
          ...
          <el-radio-group v-model="approverConfig.examineMode" class="clear">
            ...
            <br />
            <el-radio :label="3" v-if="approverConfig.settype!=2">或签(审批人中一人同意即同意)</el-radio>
          </el-radio-group>
        </div>
